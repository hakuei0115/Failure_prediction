...
def main():
    ...
    detectors = {
        key: CycleDetector(low_th=LOW_END, high_th=HIGH_ON, sensor_key=key, mode=MODE_MAP[key], on_timeout=on_timeout, fixed_duration_sec=11.0)
    ...
    for record in simulate_data_stream():
        for key, det in detectors.items():
            ...
            print(f"✅ {sensor_name} 週期 #{cid}｜{start_ts}→{end_ts}｜"
